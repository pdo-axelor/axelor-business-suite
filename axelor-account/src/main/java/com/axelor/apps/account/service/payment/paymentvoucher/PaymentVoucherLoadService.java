...
public class PaymentVoucherLoadService {
    ...
    public List<MoveLine> getMoveLines(PaymentVoucher paymentVoucher) throws AxelorException {
        ...
        String query = "self.partner = ?1 " + "and self.account.useForPartnerBalance = 't' "
                + "and self.amountRemaining > 0 " + "and self.move.statusSelect = ?3 "
                + "and self.move.ignoreInDebtRecoveryOk = 'f' " + "and self.move.company = ?2 ";
        ...
        moveLines = moveLineRepo.all().filter(query, paymentVoucher.getPartner(), paymentVoucher.getCompany(),
                MoveRepository.STATUS_VALIDATED).fetch();
    ...
    public PayVoucherDueElement createPayVoucherDueElement(MoveLine moveLine) throws AxelorException {
        ...
        BigDecimal paidAmountInElementCurrency = currencyService
                .getAmountCurrencyConvertedAtDate(move.getCompanyCurrency(), move.getCurrency(),
                        moveLine.getAmountPaid(), moveLine.getDate())
                .setScale(2, RoundingMode.HALF_EVEN);

        payVoucherDueElement.setPaidAmount(paidAmountInElementCurrency);

        payVoucherDueElement
                .setAmountRemaining(payVoucherDueElement.getDueAmount().subtract(payVoucherDueElement.getPaidAmount()));
    ...
    /**
     * Allows to load selected lines (from 1st 02M) to the 2nd O2M and dispatching
     * amounts according to amountRemainnig for the loaded move and the paid amount
     * remaining of the paymentVoucher
     * 
     ...
     * @throws AxelorException
     */
    public void completeElementToPay(PaymentVoucher paymentVoucher, PaymentVoucher paymentVoucherContext)
            throws AxelorException {
        ...
        for (PayVoucherDueElement payVoucherDueElementContext : paymentVoucherContext.getPayVoucherDueElementList()) {
            PayVoucherDueElement payVoucherDueElement = payVoucherDueElementRepo
                    .find(payVoucherDueElementContext.getId());
    ...
    public PayVoucherElementToPay createPayVoucherElementToPay(PayVoucherDueElement payVoucherDueElement, int sequence)
            throws AxelorException {
        ...
        BigDecimal amountRemainingInElementCurrency = currencyService
                .getAmountCurrencyConvertedAtDate(paymentVoucher.getCurrency(), payVoucherElementToPay.getCurrency(),
                        amountRemaining, paymentDate)
                .setScale(2, RoundingMode.HALF_EVEN);

        BigDecimal amountImputedInElementCurrency = amountRemainingInElementCurrency
                .min(payVoucherElementToPay.getRemainingAmount());

        BigDecimal amountImputedInPayVouchCurrency = currencyService
                .getAmountCurrencyConvertedAtDate(payVoucherElementToPay.getCurrency(), paymentVoucher.getCurrency(),
                        amountImputedInElementCurrency, paymentDate)
                .setScale(2, RoundingMode.HALF_EVEN);
    ...
    /**
     * Fonction vérifiant si l'ensemble des lignes à payer ont le même compte et que
     * ce compte est le même que celui du trop-perçu
     * 
    ...
    public void initFromInvoice(PaymentVoucher paymentVoucher, Invoice invoice) throws AxelorException {
        ...
        if (invoice.getCompanyBankDetails() != null) {
            ...
            companyBankDetails = Beans.get(BankDetailsService.class).getDefaultCompanyBankDetails(invoice.getCompany(),
                    invoice.getPaymentMode(), invoice.getPartner());
        }
        ...
        for (Iterator<PayVoucherDueElement> it = paymentVoucher.getPayVoucherDueElementList().iterator(); it
                .hasNext();) {
